    ...
    if st.button("☁️ タップでkintoneに送る", type="primary", use_container_width=True):
        with st.spinner("kintoneにデータを送信中..."):
            ...
            if success:
                st.success(msg)
                clear_backup()
                st.info("💡 kintoneへの送信が完了したため、一時保存データをクリアしました。")
            else:
                st.error(msg)
